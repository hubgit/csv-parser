// from https://code.google.com/p/jquery-csv/
function CSVParser(config) {
    ...
    this.parse = function(input) {
        ...
        if (_abort)
            addError('Abort', 'ParseAbort', 'Parsing was aborted by the user\'s step function', 'abort'); else {
    ...
    function handleQuote() {
        var delimBefore = (_state.i > 0 && isBoundary(_state.i - 1)) || _state.i === 0;
        var delimAfter = (_state.i < _input.length - 1 && isBoundary(_state.i + 1)) || _state.i == _input.length - 1;
        var escaped = _state.i < _input.length - 1 && _input[_state.i + 1] == '\'';

        if (_state.inQuotes && escaped) {
            _state.fieldVal += '\'';
            _state.i++;
        } else
            if (delimBefore || delimAfter)
                _state.inQuotes = !_state.inQuotes; else
    ...
    function notInQuotes() {
        if (_state.ch == _config.delimiter)
            saveValue(); else
            if ((_state.ch == '\r' && _state.i < _input.length - 1 && _input[_state.i + 1] == '\n') || (_state.ch == '\n' && _state.i < _input.length - 1 && _input[_state.i + 1] == '\r')) {
                newRow();
                _state.i++;
            } else
                if (_state.ch == '\n' || _state.ch == '\r')
    ...
    function saveValue() {
        if (_config.header) {
            if (_state.lineNum == 1 && _invocations == 1)
                _state.parsed.fields.push(_state.fieldVal); else {
    ...
    function endRow() {
        ...
        if (streaming() && (!_config.header || (_config.header && _state.parsed.rows.length > 0))) {
    ...
    function inspectFieldCount() {
        ...
        if (actual < expected)
            return addError('FieldMismatch', 'TooFewFields', 'Too few fields: expected ' + expected + ' fields but parsed ' + actual); else
            if (actual > expected)
    ...
    function addError(type, code, msg, errKey) {
        var row = _config.header ? (_state.parsed.rows.length ? _state.parsed.rows.length - 1 : undefined) : _state.parsed.length - 1;
        var key = errKey || row;
